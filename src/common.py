...
# Make cairosvg optional - only needed for visualization, not for inference
try:
...
def get_image(board: chess.Board, width, height):
    if not CAIRO_AVAILABLE:
        raise RuntimeError(
            "cairosvg is not available. This function is only needed for visualization. "
            "To use this function, install GTK3 runtime for Windows from: "
